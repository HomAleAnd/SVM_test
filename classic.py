...
for class1, class2 in class_pairs:
    ...
    train_features2, test_features2, train_labels2, test_labels2 = train_test_split(
        features, labels, train_size=0.1, random_state=algorithm_globals.random_seed
    )
    ...
    for n in range(N):
        for m in range(N):
            for k in range(K):
                for j in range(K):
                    Q_tilde[(K * n + k, K * m + j)] = 0.5 * (B ** (k + j)) * t[n] * t[m] * (kernel(data[n], data[m]) + xi) - (delta(n, m) * delta(k, j) * (B ** k))
    ...
    denominator = sum([alpha[n] * (C - alpha[n]) for n in range(N)])
    if denominator == 0:
        b = 0  # или другое значение по умолчанию
    else:
    ...
    # Проверка знаменателя
    denominator = sum([alpha2[n] * (C - alpha2[n]) for n in range(N)])
    if denominator == 0:
        b2 = 0  # или другое значение по умолчанию
    else:
