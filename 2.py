...
# Загрузка данных
iris_data = load_iris()
features = MinMaxScaler().fit_transform(iris_data.data)
labels = iris_data.target

# Пары классов
class_pairs = [(0, 1), (0, 2), (1, 2)]

# Оценка моделей для каждой пары классов
for class1, class2 in class_pairs:
    ...
    filtered_labels[filtered_labels == class1] = -1
    filtered_labels[filtered_labels == class2] = 1

    # Разделение данных
    train_features, test_features, train_labels, test_labels = train_test_split(
        filtered_features, filtered_labels, train_size=0.1, random_state=42
    )

    # Обучение и оценка QSVC
    qsvc = QSVC()
    qsvc.fit(train_features, train_labels)
    qsvc_score = qsvc.score(test_features, test_labels)
    print(f"QSVC Accuracy: {qsvc_score}")

    # Обучение и оценка VQC
    vqc = VQC(num_qubits=4)
    vqc.fit(train_features, train_labels)
    vqc_score = vqc.score(test_features, test_labels)
    print(f"VQC Accuracy: {vqc_score}")
    print("---")

    train_features2, test_features2, train_labels2, test_labels2 = train_test_split(
        features, labels, train_size=0.06, random_state=algorithm_globals.random_seed
    )
    ...
    b = sum(
        [alpha[n] * (C - alpha[n]) * (t[n] - (sum([alpha[m] * t[m] * kernel(data[m], data[n]) for m in range(N)]))) for
         n in range(N)]) / sum([alpha[n] * (C - alpha[n]) for n in range(N)])
    ...
    b2 = sum(
        [alpha2[n] * (C - alpha2[n]) * (t[n] - (sum([alpha2[m] * t[m] * kernel(data[m], data[n]) for m in range(N)])))
         for n in range(N)]) / sum([alpha2[n] * (C - alpha2[n]) for n in range(N)])
